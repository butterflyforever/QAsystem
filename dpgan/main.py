...
tf.flags.DEFINE_integer('vocab_size', 80000, 'Size of vocabulary. These will be read from the vocabulary file in order. If the vocabulary file contains fewer words than this number, or if this number is set to 0, will take all words in the vocabulary file.')
    ...
    while epoch < max_run_epoch:
        ...
        while step < len(batches):
            ...
            if train_step % 1000 == 0:
                ...
                t1 = time.time()
                tf.logging.info('seconds for %d training dirscriminator step: %.3f ', train_step, (t1 - t0) / 1000)
                t0 = time.time()
                tf.logging.info('loss: %f', loss_window / 100)  # print the loss to screen
                loss_window = 0.0
...
def main(unused_argv):
    ...
    for key,val in FLAGS.__flags.items():
        if key in hparam_list:
            hps_dict[key] = val # value # add it to the dict # aloha
    ...
    if FLAGS.mode == 'train_generator':
    ...
    elif FLAGS.mode == 'generator_test':
        # print("Start pre-training ......")
        ge_model = Generator(hps_generator, vocab)
        sess_ge, saver_ge, train_dir_ge = setup_training_generator(ge_model)

        generated = Generated_sample(ge_model, vocab, batcher, sess_ge)
        print("Start loading pre-trained generator .....")
        # run_pre_train_generator(ge_model, batcher, 1000, sess_ge, saver_ge, train_dir_ge)
        util.load_ckpt(saver_ge, sess_ge, ckpt_dir="train-generator")
        print("finish load train-generator")
        ...
        while inputs != "close":
            ...
            enc_input = np.resize([enc_input] * 64, (64,1,40))
            dec_batch = np.resize([dec_batch] * 64, (64,1,40))  # aloha
